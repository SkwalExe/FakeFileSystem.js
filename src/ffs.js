...
class FFS {
    constructor() {
        ...
        this.getPath = function(path) {
            ...
            var currentDirectory = this.tree[0];
            ...
            for (var i = 0; i < pathArr.length; i++) {
                ...
                if (i == pathArr.length - 1) {
                    ...
                    if (result.result = currentDirectory.content.find(e => e.name == file))
                        result.success = true;
                    else {
        ...
        this.isDir = function(path) {
            ...
            // else return false
            if (result.success)
                return result.result.type == "directory";
            return false;
        };
         ...
         * @example
         *
         * var result = FFS.writeFile("/test.txt", "Hello World!")
        ...
        this.writeFile = function(path, content, append = false) {
            ...
            if (this.fileExists(path)) {
                ...
                if (this.isRegularFile(path)) {
                    var file = this.getPath(path).result;
         ...
         *
         * var result = FFS.writeFile("/DOES_NOT_EXIST/", "test.txt")
         * if (!result.success)
        ...
        this.createFile = function(path, filename, content = "") {
            ...
            var parent = this.getPath(path).result;
         ...
         * @example
         *
         * var result = FFS.createDirectory("/", "myDirectory")
         * if (!result.success)
         *   return console.log(result.error)
        ...
        this.simplifyPath = function(path) {
            ...
            if (path.startsWith("./"))
                path = this.simplifyPath(this.currentDirectory + "/" + path.substring(2));

            // replace leading '../' with absolute path
            if (path.startsWith("../")) {
                var parent = this.getPath(this.currentDirectory).result;
            ...
            if (!path.startsWith("/"))
                return this.simplifyPath(this.currentDirectory + "/" + path);
         ...
         * @example
         *
         * var result = FFS.deleteFile("/myDirectory/test.txt")
         * if (!result.success)
         *     return console.log(result.error)
        ...
        this.delete = function(path) {
            ...
            // get the parent directory
            var parent = this.getPath(file.parent).result;
         ...
         * @example
         *
         * var result = FFS.getParent("/myDirectory/test.txt")
         * if (!result.success)
         *   return console.log(result.error)
         ...
         * @example
         *
         * var file = FFS.getPath("/myDirectory/test.txt").result
         ...
         * @example
         *
         * var result = FFS.copy("/myDirectory/test.txt", "/myDirectory/test2.txt")
        ...
        this.copy = function(source, destination) {
            ...
            // get source file
            var file = this.getPath(source).result;
            ...
            if (!this.fileExists(destination)) {
                ...
                var destinationParent = this.getParent(destination).result;
                ...
                var destination = this.getPath(destination).result;
                var newFile = {
                    name: file.name,
                    type: file.type,
                    content: file.content,
        ...
        this.move = function(source, destination) {
            ...
            let operation = this.copy(source, destination);
